...
// Helper to execute Python code in Frappe context
async function execFrappePython(pythonCode, stepName) {
    try {
        // Use heredoc to pass Python code directly
        const command = `cd ${DOCKER_COMPOSE_DIR} && docker compose exec -T ${DOCKER_SERVICE} bash -c "bench --site ${SITE_NAME} console <<'PYTHON_EOF'
${pythonCode}
PYTHON_EOF
"`;
...
try:
    ...
    print(f"WARNING: Settings init failed: {str(e)}")
