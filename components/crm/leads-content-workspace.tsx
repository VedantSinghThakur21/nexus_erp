...
function getStageColor(stage: string): string {
    const colors: Record<string, string> = {
        'Lead': 'bg-secondary text-foreground dark:bg-muted/30 dark:text-foreground',
        ...
        'Do Not Contact': 'bg-secondary text-muted-foreground dark:bg-muted/30 dark:text-muted-foreground'
    }
    return colors[stage] || 'bg-secondary text-muted-foreground dark:bg-muted/30 dark:text-muted-foreground'
}
...
export function LeadsContentWorkspace({ leads }: LeadsContentWorkspaceProps) {
    ...
    return (
        <div className="app-shell text-foreground min-h-screen">
                                                            ...
                                                            <div className="flex-1 space-y-4 overflow-y-auto no-scrollbar pr-1">
                                                                {stageLeads.map(lead => {
                                                                    ...
                                                                    return (
                                                                        <div
                                                                            key={lead.name}
                                                                            className={`bg-card p-5 rounded-xl border border-border shadow-sm hover:shadow-md transition group ${isConverted || !canEdit ? 'opacity-60 cursor-not-allowed' : 'cursor-move'
                                                                                }`}
            ...
            {/* Dark Mode Toggle */}
            <button
                className="fixed bottom-6 right-6 w-12 h-12 bg-card rounded-full shadow-2xl flex items-center justify-center border border-border text-muted-foreground hover:text-primary transition-all z-50 hover:scale-110 active:scale-95"
                onClick={() => {
