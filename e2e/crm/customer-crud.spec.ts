...
test('Create a new customer -> appears in customers list', async ({ request, page }) => {
...
test('Delete customer with linked transactions -> blocked (referential integrity)', async ({ request }) => {
  test.setTimeout(3 * 60_000)
  ...
  // 1) Create customer
  const createCustomer = await frappeProxy<any>(request, 'frappe.client.insert', {
    method: 'POST',
    body: {
  ...
  const so = await frappeProxy<any>(request, 'frappe.client.insert', {
    method: 'POST',
    body: {
      doc: {
        doctype: 'Sales Order',
  ...
  const del = await frappeProxy<any>(request, 'frappe.client.delete', {
    method: 'POST',
