...
export async function getDashboardStats(accessibleModules: string[] = []) {
  ...
  try {
    ...
    const getAmount = (row: any): number => {
      const raw = row?.base_grand_total ?? row?.grand_total ?? row?.opportunity_amount ?? 0
    ...
    if (accessibleModules.includes('quotations')) {
      const [
        ...
        quotationsWonLastMonthRaw,
        quotationsLostLastMonthRaw,
      ] = await Promise.all([
      ...
      const quotationsWonMTD = toNumber(quotationsWonMTDRaw)
      const quotationsLostMTD = toNumber(quotationsLostMTDRaw)
      const quotationsWonLastMonth = toNumber(quotationsWonLastMonthRaw)
      const quotationsLostLastMonth = toNumber(quotationsLostLastMonthRaw)
...
export async function getOpportunities(accessibleModules: string[] = []) {
  ...
  try {
    ...
    const quotes = await frappeRequest('frappe.client.get_list', 'GET', {
      doctype: 'Quotation',
      fields: '["name", "customer_name", "party_name", "base_grand_total", "grand_total", "status", "modified"]',
    ...
    return quotes.map((q: any) => ({
      ...
      opportunity_amount: typeof (q.base_grand_total ?? q.grand_total) === 'number' ? (q.base_grand_total ?? q.grand_total) : Number(q.base_grand_total ?? q.grand_total) || 0,
...
export async function getRecentActivities(accessibleModules: string[] = []) {
  ...
  try {
    ...
    const [closedResult, quotesResult] = await Promise.all([
      accessibleModules.includes('sales-orders')
        ? frappeRequest('frappe.client.get_list', 'GET', {
          doctype: 'Sales Order',
    ...
    if (Array.isArray(closedResult)) {
      closedResult.forEach((deal: any) => {
        allActivities.push({
          type: 'closed-deal',
    ...
    if (Array.isArray(quotesResult)) {
      quotesResult.forEach((quote: any) => {
        allActivities.push({
          type: 'new-lead',
...
export async function getSalesPipelineFunnel(accessibleModules: string[] = []) {
  ...
  try {
    ...
    const [leadsCount, quotes, orders] = await Promise.all([
      accessibleModules.includes('crm')
        ? safeFetch(() => frappeRequest('frappe.client.get_count', 'GET', {
          doctype: 'Lead',
          filters: '[["status", "!=", "Converted"], ["status", "!=", "Do Not Contact"]]',
        }), 0)
        : Promise.resolve(0),
      accessibleModules.includes('quotations')
        ? safeFetch(() => frappeRequest('frappe.client.get_list', 'GET', {
          doctype: 'Quotation',
          fields: '["base_grand_total", "grand_total", "status"]',
    ...
    if (Array.isArray(quotes)) {
      quotes.forEach((q: any) => {
        const amount = typeof (q.base_grand_total ?? q.grand_total) === 'number'
          ? (q.base_grand_total ?? q.grand_total)
          : Number(q.base_grand_total ?? q.grand_total) || 0
        if (q.status === 'Draft') {
          stages['Draft Quote'].count++
          stages['Draft Quote'].value += amount
        } else {
          stages['Open Quote'].count++
          stages['Open Quote'].value += amount
        }
      })
    }
    ...
    if (Array.isArray(orders)) {
      orders.forEach((o: any) => {
        const amount = typeof (o.base_grand_total ?? o.grand_total) === 'number'
          ? (o.base_grand_total ?? o.grand_total)
          : Number(o.base_grand_total ?? o.grand_total) || 0
        stages['Sales Order'].count++
        stages['Sales Order'].value += amount
      })
    }
...
export async function getDealsByStage(accessibleModules: string[] = []) {
  ...
  try {
    ...
    const [quotes, wonCount] = await Promise.all([
      frappeRequest('frappe.client.get_list', 'GET', {
        doctype: 'Quotation',
        fields: '["status"]',
        filters: '[["status", "in", ["Draft", "Open"]]]',
      ...
      frappeRequest('frappe.client.get_count', 'GET', {
        doctype: 'Quotation',
        filters: `[["status", "=", "Ordered"], ["modified", ">=", "${monthStart}"]]`,
      }),
    ])
