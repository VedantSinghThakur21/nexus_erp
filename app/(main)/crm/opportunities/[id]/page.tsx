...
export default async function OpportunityDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const stageColors: Record<string, string> = {
    'Prospecting': 'bg-secondary text-foreground',
        ...
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
                    ...
                    <tbody>
                      {opportunity.items.map((item: Record<string, any>, idx: number) => (
                        <tr key={idx} className="border-b border-border/40 hover:bg-muted/30 transition-colors">
                          ...
                          <td className="px-4 py-3 text-right text-foreground">₹{(item.rate || 0).toLocaleString('en-IN')}</td>
                          <td className="px-4 py-3 text-right font-medium text-foreground">₹{(item.amount || 0).toLocaleString('en-IN')}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
            ...
            <div className="mt-6">
              ...
              <div className="mt-2 p-3 bg-muted rounded-md text-sm text-foreground min-h-[80px] whitespace-pre-wrap">
