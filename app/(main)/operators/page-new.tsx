...
export default function OperatorsPage() {
  ...
  const kpis = useMemo(() => {
    ...
    const avgUtil = total > 0 ? Math.round((onField / total) * 100) : 0;
    return {
      total,
      onField,
      avgUtil: `${avgUtil}%`,
  ...
  const filteredOperators = operators.filter((op) => {
    const matchesSearch =
      op.employee_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      op.designation?.toLowerCase().includes(searchTerm.toLowerCase()) ||
      op.cell_number?.toLowerCase().includes(searchTerm.toLowerCase());
    const matchesRole = roleFilter === "All Roles" || op.designation === roleFilter;
    const matchesStatus = statusFilter === "Availability Status" || op.status === statusFilter;
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <div className="flex-1 overflow-y-auto p-8 max-w-[1600px] mx-auto w-full space-y-8">
          <div className="flex flex-col gap-1">
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Operators</h2>
            <p className="text-sm text-slate-500 dark:text-slate-400">Manage drivers, riggers, and field staff intelligence</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div className="bg-navy-deep dark:bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-xl">
              ...
              <div className="text-[11px] text-slate-500">No active records</div>
            </div>
            <div className="bg-navy-deep dark:bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-xl">
              <div className="flex justify-between items-center">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">On-Field</span>
              ...
              <div className="text-[11px] text-emerald-500 flex items-center gap-1">
                <span className="material-symbols-outlined text-[14px]">bolt</span> Live Tracking Active
              </div>
            </div>
            <div className="bg-navy-deep dark:bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-xl">
              <div className="flex justify-between items-center">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Avg. Utilization</span>
                <div className="w-10 h-10 rounded-lg bg-purple-500/10 flex items-center justify-center">
                  <span className="material-symbols-outlined text-purple-500">trending_up</span>
                </div>
              </div>
              <div className="text-4xl font-bold text-white">{kpis.avgUtil}</div>
              <div className="text-[11px] text-slate-500">Baseline calculation...</div>
            </div>
            <div className="bg-navy-deep dark:bg-slate-900 p-6 rounded-xl border border-slate-800 flex flex-col gap-4 shadow-xl">
              <div className="flex justify-between items-center">
                <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">AI Efficiency Score</span>
                <div className="w-10 h-10 rounded-lg bg-amber-500/10 flex items-center justify-center">
                  <span className="material-symbols-outlined text-amber-500">electric_bolt</span>
                </div>
              </div>
              <div className="text-4xl font-bold text-white">{kpis.aiScore}</div>
          ...
          <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 p-6 rounded-xl flex flex-wrap items-center justify-between gap-6 shadow-sm">
            <div className="flex items-center gap-6">
              ...
              <div>
                <h4 className="text-lg font-bold text-blue-900 dark:text-blue-300">AI Workforce Intelligence</h4>
                <p className="text-sm text-blue-700/80 dark:text-blue-400/80 mt-0.5">Predicted Demand: <span className="font-semibold">Stable</span> over the next 7 days.</p>
              </div>
            </div>
            <div className="flex items-center gap-8">
              <div className="hidden lg:block w-48 h-12">
                <svg className="w-full h-full text-blue-400 dark:text-blue-600" preserveAspectRatio="none" viewBox="0 0 100 30">
                  <path d="M0,25 Q10,25 20,20 T40,15 T60,18 T80,10 T100,12" fill="none" stroke="currentColor" strokeWidth="2.5"></path>
                </svg>
              </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-3">
                Active Crew <span className="bg-slate-100 dark:bg-slate-800 text-slate-500 text-xs px-2.5 py-1 rounded-full">{filteredOperators.length}</span>
              </h3>
            ...
            {filteredOperators.length === 0 ? (
              <div className="bg-white dark:bg-navy-deep border border-slate-200 dark:border-slate-800 rounded-2xl min-h-[450px] flex flex-col items-center justify-center text-center p-12 shadow-sm">
                <div className="w-20 h-20 bg-slate-50 dark:bg-slate-900/80 rounded-full flex items-center justify-center mb-8">
                  <span className="material-symbols-outlined text-slate-200 dark:text-slate-800 text-[48px]">person_off</span>
                </div>
                <h4 className="text-2xl font-bold text-slate-800 dark:text-white mb-3">No operators found</h4>
                <p className="text-slate-500 dark:text-slate-400 max-w-md mb-10 leading-relaxed text-base">Add your first crew member to start assigning them to projects and tracking their efficiency with our AI-driven workforce intelligence engine.</p>
                <CreateOperatorDialog />
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredOperators.map((op) => (
                  <div key={op.name} className="bg-white dark:bg-navy-deep border border-slate-200 dark:border-slate-800 rounded-xl p-6 shadow-sm flex flex-col gap-4">
