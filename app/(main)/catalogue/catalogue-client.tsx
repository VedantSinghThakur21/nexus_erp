...
export function CatalogueClient(props: { initialItems: Item[] }) {
  ...
  const handleQuickView = async (itemCode: string) => {
    ...
    try {
      const [details, analytics] = await Promise.all([getItemDetails(itemCode), getItemRentalAnalytics(itemCode)])
  ...
  return (
    <div className="app-shell">
        ...
        <div className="app-container w-full space-y-8">
          ...
          <div className="grid grid-cols-12 gap-8">
            <aside className="col-span-12 lg:col-span-3 xl:col-span-2 space-y-8">
              <div>
                <h5 className="text-[11px] uppercase font-bold tracking-widest text-muted-foreground  mb-4 border-b dark:border-slate-800 pb-2">
                  Categories
                </h5>
                ...
                {filteredItems.length === 0 ? (
                  <div className="col-span-2 bg-card dark:bg-background border border-slate-200 dark:border-slate-800 rounded-xl p-12 text-center">
                    ...
                    <p className="text-sm text-muted-foreground  mb-6">
                      {allItems.length === 0 ? 'Get started by adding your first item to the catalogue.' : 'Try adjusting your filters or search query.'}
                    </p>
                  ...
                  paginatedItems.map((item) => (
                    <div
                      key={item.item_code}
                      className="bg-card dark:bg-background border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden shadow-sm group hover:shadow-md transition"
                    >
                      <div className="p-6">
                        <div className="flex justify-between items-start mb-2">
                          ...
                          <span className="bg-blue-100 dark:bg-blue-900/40 text-primary text-[10px] font-bold px-2 py-1 rounded uppercase">
                            {item.item_group}
                          </span>
                        </div>
                      ...
                      <div className="flex border-t border-slate-100 dark:border-slate-800">
                        <button
                          onClick={() => void handleQuickView(item.item_code)}
            ...
            <aside className="col-span-12 lg:col-span-3 xl:col-span-3 space-y-6">
              ...
              <div className="bg-gradient-to-br from-midnight-blue to-slate-800 rounded-xl p-6 text-white shadow-xl">
                ...
                <p className="text-sm text-slate-300 mb-4">
                  Our enterprise support team is available 24/7 to help you optimize your catalogue.
                </p>
                <button className="w-full py-2.5 bg-background text-midnight-blue rounded-lg font-bold text-sm hover:bg-slate-100 transition">
                  Contact Support
                </button>
              </div>
            </aside>
      ...
      {editingItem && (
        <EditItemDialog item={editingItem} open={!!editingItem} onClose={() => setEditingItem(null)} onUpdated={() => loadItems()} />
      )}
              ...
              {isLoadingDetails ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      <span className="font-semibold">Category:</span> {selectedItem.item_group}
                    </div>
                    <div>
                      <span className="font-semibold">Unit:</span> {selectedItem.uom || 'Unit'}
                    </div>
                    <div>
                      <span className="font-semibold">Stock Item:</span> {selectedItem.is_stock_item ? 'Yes' : 'No'}
                    </div>
                    <div>
                      <span className="font-semibold">Has Serial No:</span> {selectedItem.has_serial_no ? 'Yes' : 'No'}
                    </div>
                  </div>
