            ...
            <div className="grid grid-cols-2 gap-6">
              ...
              <Card className="rounded-2xl border border-gray-200 bg-white shadow-sm flex-1 flex flex-col">
                <CardHeader className="px-4 py-2 flex flex-row items-center gap-2">
                  <BarChart3 className="h-3.5 w-3.5 text-[#A0AEC0]" />
                  <CardTitle className="text-xs font-bold text-gray-700 uppercase tracking-wider">Sales Funnel</CardTitle>
                </CardHeader>
                    ...
                    {funnelData.map((stage, index) => (
                      <div key={stage.name}>
                        <div className="flex justify-between items-center mb-1">
                          <span className="text-[10px] font-bold text-gray-500 uppercase">{stage.name}</span>
                          <span className="text-[10px] font-semibold text-gray-400">
                            {formatIndianCurrency(stage.value)}
                          </span>
                        </div>
                        {/* Full width bar */}
                        <div className="w-full h-7 rounded-md overflow-hidden bg-gray-100 flex items-center">
                          <div
                            className="bg-[#3B82F6] h-full flex items-center px-2"
                            style={{ width: `${stage.percentage}%` }}
                          >
                            <span className="text-xs font-bold text-white whitespace-nowrap">{stage.count} Deals</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              <Card className="rounded-2xl border border-gray-200 bg-white shadow-sm flex flex-col">
                <CardHeader className="px-4 py-2 flex flex-row items-center gap-2">
                  ...
                  <CardTitle className="text-xs font-bold text-gray-700 uppercase tracking-wider">Leads Source</CardTitle>
                </CardHeader>
                <CardContent className="px-4 pb-3 pt-1 flex items-center justify-between gap-4">
                  {(() => {
                        ...
                        <div className="flex flex-col gap-1.5 flex-1">
                          {leadSources.slice(0, 5).map((source, index) => (
                            <div key={source.source} className="flex items-center gap-2">
                              ...
                              ></div>
                              <span className="text-xs text-gray-700 truncate flex-1">{source.source}</span>
